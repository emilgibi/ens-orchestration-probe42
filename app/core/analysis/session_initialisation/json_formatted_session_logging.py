...
async def format_csv_report(data, session):
    ...
    ens_id = await get_dynamic_ens_data("upload_supplier_data", upload_meta_cols,None,
                                           session_id_value, session)
    # print(f"data type{ens_id}, \n {type(ens_id)}")
    meta_cols=[]
    for ens in ens_id:
        meta_cols.extend(await get_join_dynamic_ens_data("upload_supplier_data", "supplier_master_data",ens["ens_id"], session_id_value, session))
    meta_cols = [
